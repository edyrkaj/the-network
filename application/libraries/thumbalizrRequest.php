...
class ThumbalizrRequest {
	...
	private function build_request($url) {
		...
		"url=".$url;
		$this->local_cache_file=md5($url).".".$this->encoding;
		$this->local_cache_thumb=md5($url)."-thumb.".$this->encoding;
	...
	private function save() { 
		...
		if ($this->img && $this->use_local_cache===TRUE && $this->headers['Status']=="OK") {
			...
			$thumbimage = $this->local_cache_subdir."/".$this->local_cache_thumb;
